...
- **Where stored:** one file **`secret/bank_secret.key`** at the repo root,
  **committed to the repository** (the repo is private). This gives **one
  consistent secret** across every build, CI runner and developer — no per-machine
  divergence, no manual backup (git is the backup), and dev builds open production
  licensed banks. The bake tool reads it (`--secret-file`, default
  `secret/bank_secret.key`); CMake compiles it into
  `build/generated/bank_secret_generated.h` (`constexpr uint8_t kBankSecret[32]`,
  a build artifact, gitignored) which the engine links.
- **Build fallback, all targets:** `tools/gen-bank-secret.py` creates the file
  with 32 random bytes **only if it is missing** (idempotent — never overwrites
  the committed key). It runs from **both** the Makefile (`configure`
  prerequisite) **and** CMake configure (`execute_process`), so any `make`/`cmake`
  target works. Normally the committed key is present and the generator is a no-op
  safety net; if the key is ever deleted it regenerates a *different* random key
  (which would break previously-shipped banks — see rotation).
- **Rotation:** changing `secret/bank_secret.key` invalidates every previously
  shipped licensed bank (new player builds can't open them) → re-bake + redistribute.
  Because the secret is in git history, treat the private repo accordingly; were
  it ever exposed, rotate and re-bake.
