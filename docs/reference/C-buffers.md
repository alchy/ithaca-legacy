...
- **Standalone:** `audio_sample_rate` ze `state.json` (default 48000) → `EngineConfig::sample_rate` → request do miniaudio (`config.sampleRate` v `AudioDevice::start`). Mění se **jen ručně v JSONu**, aplikuje se při startu; GUI ho jen zobrazuje read-only (patička). Žádný runtime SR setter neexistuje (ani ho nepotřebujeme, dokud SR nemění host).
...
**BUFFER selektor (stránka SYS):** `audio_block_size` ze `state.json` (default 256), runtime měnitelný comboem `{32, 64, 128, 256, 512, 1024, 2048, 4096, 8192}`. Změna jde přes `AppContext::setAudioBlockSize(int)`:
...
**DSP load metr (`Engine::processBlock`, audio thread):** na konci bloku se změří `dt = render_us` (vůči `block_t0` z `nowMicros()` po `bank_loading_` guardu), `period_us = n_samples*1e6 / sample_rate`, `load = dt / period`. Peak-hold s decay ~0.5 s se ukládá do atomiky `dsp_load_peak_`; při `load >= 1.0` (blok minul deadline → riziko dropoutu) se orazítkuje `last_overload_us_`. Gettery `Engine::dspLoadPeak()` a `Engine::overloadRecent(ms)` (relaxed atomiky — single-writer audio, reader GUI) drží testy; GUI čte tytéž hodnoty ze snapshotu `EngineDiag` (`dsp_load_peak`, `overload_age_ms`). GUI je zobrazuje na stránce **PLAY** ve sloupci `DSP` (`<peak%>`, vedle VOICES/RESO/RING/RING RESO/PEAK dB/SUSTAIN, viz oblast H — GUI); stav ringů je ve sloupcích `RING` a `RING RESO` tamtéž. BUFFER selektor na SYS ukazuje jen počet framů (ms latence `frames*1000/SR` se kvůli místu nezobrazuje, ale platí jako koncept). Perioda počítaná z `n_samples` → metr se přizpůsobí jakémukoli bufferu, i když ho řídí host.
...
| `int StreamEngine::capacityFrames() const noexcept` | libovolné | — → kapacita jednoho ringu | diagnostika, výpočet refill prahu | — | — | `ring_capacity_frames` z `EngineConfig` (default 8192, ~170 ms @ 48 kHz). |
...
- Loader (`sample_store.cpp`, volné funkce `prepareSampleFile` / `loadBank`) volá `readWavRange` off-RT pro preload hlavy samplu a resonanční okno při načítání banky.
