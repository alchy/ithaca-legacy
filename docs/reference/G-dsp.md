...
**AGC → BBE → Limiter**; každý stupeň je instancí abstraktní třídy `DspStage` a
...
| `engine/dsp/bbe.h` | Deklarace BBE Sonic Maximizer stupně | `BBE` |
| `engine/dsp/bbe.cpp` | Implementace `process()` a `applyParams_()` pro BBE | — |
...
GUI-facing rozhraní. Implementují ho `DspStage` (AGC, BBE, Limiter) i `VoicePage`
...
|--------------------|--------|----------------|---------|-------------|-------|
| `name() → const char*` | libovolné | — → název stage | GUI panely | — | Název stage pro záhlaví stránky (`"AGC"`, `"BBE"`, `"LIMITER"`) |
...
| `meter(value&, label&) → bool` | GUI | — → (hodnota, popis) nebo false | diagnostický panel | konkrétní `atomic::load` | AGC vrací aktuální gain (lineární), Limiter vrací gain reduction (dB), BBE vrací `false` |

#### `struct DspStage : IParamPage`

Přidává tři audio-thread metody k `IParamPage`. Implementují je AGC, BBE, Limiter.
...
| `biquad_tick(x, c, s) → float` | audio | vzorek + koeficienty + stav → výstup | `BBE::process` (×4 na vzorek) | — | `x`: vstupní vzorek; `c`: `BiquadCoeffs`; `s`: `BiquadState` (mutable) | DF-II transponovaná forma; aktualizuje `s.s1`, `s.s2` in-place; jedna iterace je 5 MAC |
| `rbj_high_shelf(fc, gain_db, sr) → BiquadCoeffs` | off-RT / lazy v process | střed [Hz], boost/cut [dB], sr → koeficienty | `BBE::applyParams_` | `std::pow`, `std::cos`, `std::sin`, `std::sqrt` | `fc`: 5 000 Hz (BBE DEFINITION); `gain_db`: 0–12 dB | Audio EQ Cookbook RBJ high-shelf; přepočítá se jen při změně `def_db_` |
| `rbj_low_shelf(fc, gain_db, sr) → BiquadCoeffs` | off-RT / lazy v process | střed [Hz], boost/cut [dB], sr → koeficienty | `BBE::applyParams_` | `std::pow`, `std::cos`, `std::sin`, `std::sqrt` | `fc`: 180 Hz (BBE BASS); `gain_db`: 0–10 dB | Audio EQ Cookbook RBJ low-shelf; přepočítá se jen při změně `bass_db_` |
...
|--------------------|--------|----------------|---------|-------------|-----------|------------|
| `prepare(sr, max_block)` | off-RT | sample rate, max block | `Engine::prepare` (implicitně přes `dsp_`) | `DspStage::prepare` na každý stupeň | — | Propaguje `prepare` na AGC, BBE, Limiter v pořadí `stages_[0..2]` |
...
| `stage(i) → DspStage&` | libovolné | index (0=AGC, 1=BBE, 2=LIMITER) → reference | GUI (`main.cpp`, `app_context.cpp`), testy | — | Umožňuje GUI přistupovat k `IParamPage` rozhraní stage bez znalosti konkrétního typu |

Privátní pole: `agc_`, `bbe_`, `lim_` (konkrétní instance), `stages_[3]` (pole
...
### `engine/dsp/bbe.h` + `bbe.cpp`

Zjednodušený BBE Sonic Maximizer: dva nezávislé RBJ biquad shelving filtry na každý
kanál. DEFINITION = high-shelf boost nad 5 kHz; BASS = low-shelf boost pod 180 Hz.
Oba se aplikují sériově: nejprve high-shelf, pak low-shelf.

#### Parametry (kParams)

| Index | id | Label | Rozsah | Výchozí | Formát |
|-------|----|-------|--------|---------|--------|
| 0 | `definition` | DEFINITION | 0 – 12 dB | 0 dB | `"%.1f dB"` |
| 1 | `bass` | BASS | 0 – 10 dB | 0 dB | `"%.1f dB"` |

#### Metody

| Metoda (signatura) | Vlákno | Vstup → výstup | Volá ji | Volá (proč) | Parametry | Vysvětlení |
|--------------------|--------|----------------|---------|-------------|-----------|------------|
| `prepare(sr, max_block)` | off-RT | sample rate | `DspChain::prepare` | `applyParams_(true)`, `reset()` | — | Uloží `sr_`, vynutí přepočet obou biquad koeficientů, vynuluje stav |
| `reset()` | off-RT | — | `DspChain::reset` | — | Vynuluje čtyři `BiquadState` struktury (`def_l_`, `def_r_`, `bass_l_`, `bass_r_`) = vymaže zpoždění filtrů |
| `get(i) / set(i,v)` | GUI / libovolné | index, hodnota | `renderParamPage`, persistence | `atomic::load/store(relaxed)` | i=0 definition dB, i=1 bass dB | Triviální; `set()` klampuje |
| `enabled() / setEnabled(on)` | libovolné / GUI | — / bool | `DspChain::process`, `renderConfigPanel` | `atomic::load/store(relaxed)` | — | Výchozí `false` |
| `meter(...) → false` | GUI | — → false | diagnostický panel | — | — | BBE nemá žádný výstupní metr |
| `applyParams_(force)` | audio (z process) | bool force | `process()`, `prepare()` | `rbj_high_shelf`, `rbj_low_shelf` | `force`: vždy přepočítat | Lazy přepočet: `d != last_def_db_` → `def_c_ = rbj_high_shelf(5000, d, sr_)`; `b != last_bass_db_` → `bass_c_ = rbj_low_shelf(180, b, sr_)`. Inicializační sentinels: `last_def_db_ = 1e9f`, `last_bass_db_ = 1e9f` |
| `process(L, R, n)` | audio | in-place stereo buffer | `DspChain::process` | `applyParams_(false)`, `biquad_tick` (×4 na vzorek) | — | Viz níže |

**`BBE::process()` — podrobně:**

1. Pokud `!enabled_` → return.
2. `applyParams_(false)` — lazy přepočet biquad koeficientů při změně.
3. Per-vzorek smyčka (délka n):
   - `L[i] = biquad_tick(L[i], def_c_, def_l_)` — high-shelf na L
   - `R[i] = biquad_tick(R[i], def_c_, def_r_)` — high-shelf na R
   - `L[i] = biquad_tick(L[i], bass_c_, bass_l_)` — low-shelf na L
   - `R[i] = biquad_tick(R[i], bass_c_, bass_r_)` — low-shelf na R
...
BBE, LIMITER). `renderConfigPanel` zobrazuje LED enable toggle — volá
...
(`target_`, `release_ms_`, `floor_`, `def_db_`, `bass_db_`, `thr_db_`, `rel_ms_`,
...
Přesto `AGC::process()` i `BBE::process()` začínají `if (!enabled_) return`. Toto
...
`BBE::applyParams_` volá `rbj_high_shelf` / `rbj_low_shelf`, které volají
