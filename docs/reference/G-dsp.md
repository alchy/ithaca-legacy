...
**Convolver** — simulace těla nástroje pro blízko-snímané vzorky. **Direct time-domain FIR** konvoluce, mono IR aplikovaná na L i R nezávisle, wet/dry **MIX** (default 0,25 = subtilní). **První** stage v řetězci (tělo před masteringem). Fixní history ring `kMaxIr=8192` (~170 ms @48k); **RT-safe 2-slot IR swap** (atomicky publikovaný `active_` index, audio čte druhý slot až po dočtení) → změna IR za běhu bez realokace ringu. IR se při `setIR` peak-normalizuje. `process` alloc-free; `setIR`/generování off-RT.
