...
| `engine/dsp/dsp_chain.h` | Kontejner čtyř pevně zapojených stupňů; iteruje jen enabled | `DspChain` |
...
| `engine/dsp/convolver.{h,cpp}` | Convolver (cabinet/body sim): direct FIR, mono IR na L/R, MIX, 2-slot IR swap | `Convolver` |
| `engine/dsp/ir_wav.{h,cpp}` | Mono IR WAV loader (float32/int16, resample, cap) | `loadIrWavMono` |
| `engine/dsp/ir_modal.{h,cpp}` | Synteticky modalni soundboard IR generator (Chabassier/RR_9530) | `generateModalIr` |
...
Přidává tři audio-thread metody k `IParamPage`. Implementují je Convolver, AGC, Enhancer, Limiter.
...
|--------------------|--------|----------------|---------|-------------|-----------|------------|
| `prepare(sr, max_block)` | off-RT | sample rate, max block | `Engine::prepare` (implicitně přes `dsp_`) | `DspStage::prepare` na každý stupeň | — | Propaguje `prepare` na Convolver, AGC, Enhancer, Limiter v pořadí `stages_[0..3]` |
| `reset()` | off-RT | — | `Engine::reset` (pokud existuje) | `DspStage::reset` na každý stupeň | — | Vymaže stav všech čtyř stupňů |
| `process(L, R, n)` | audio | in-place stereo buffer, délka n | `Engine::processBlock` (krok 3b/5) | `DspStage::process` jen pro `enabled()` stupně | — | Iteruje `stages_[0..3]`; disabled stupeň přeskočí (no-op passthrough) |
| `stageCount() → int` | libovolné | — → 4 | GUI (`main.cpp`) | — | Konstanta; vrací vždy 4 |
| `stage(i) → DspStage&` | libovolné | index (0=CONVOLVER, 1=AGC, 2=ENHANCER, 3=LIMITER) → reference | GUI (`main.cpp`, `app_context.cpp`), testy | — | Umožňuje GUI přistupovat k `IParamPage` rozhraní stage bez znalosti konkrétního typu |

Privátní pole: `convolver_`, `agc_`, `enhancer_`, `lim_` (konkrétní instance),
`stages_[4]` (pole ukazatelů; pořadí fixní).
...
5. `target_gain = target_rms / rms`, klip na `[gain_floor, 1.0]`; pokud
   `rms ≤ 1e-6f`, `target_gain = 1.f` (ticho → cíl 1,0, takže gain se RELEASE
   koeficientem plynule vrací zpět k 1,0 — není to „beze změny").
...
### `engine/dsp/convolver.{h,cpp}` (Convolver — cabinet/body sim)

**Convolver** — simulace těla nástroje pro blízko-snímané vzorky. **Direct time-domain FIR** konvoluce, mono IR aplikovaná na L i R nezávisle, wet/dry **MIX** (default 0,15 = subtilní). **První** stage v řetězci (tělo před masteringem). Fixní history ring `kMaxIr=8192` (~170 ms @48k); **RT-safe 2-slot IR swap s ack protokolem** (atomicky publikovaný `active_` index + atomic `seen_`: `process()` na začátku publikuje čtený slot, na všech návratových cestách vrací −1 = „audio nečte"; `setIR` před přepsáním neaktivního slotu bounded-čeká s deadline 250 ms, dokud ho audio thread čte) → změna IR za běhu bez realokace ringu a bez rizika dealokace vectoru pod in-flight blokem. `prepare()` resetuje `seen_` na −1. IR se při `setIR` peak-normalizuje. `process` alloc-free; `setIR`/generování off-RT.

`prepare()` **zachovává vybrané IR**: přečte `cur_choice_` a přegeneruje IR pro tuto volbu (přes `selectChoice`, aby mapování index→preset zůstalo na jednom místě). Dříve tu bylo natvrdo `cur_choice_ = 0`, takže každé re-prepare — a to volá `Engine::setBlockSize()` i změna sample rate — tiše přepnulo IR zpět na `Body soft`. V GUI se to projevovalo tak, že změna BUFFER shodila uživatelův výběr IR a zrcadlení do `GuiState` tu nulu ještě uložilo do `state.json`.
...
Parametry (vše 0..1): index 0 = `MIX` (wet/dry, runtime; nízký = subtilní tělo), 1 = `DECAY` (tvarování doznění IR), 2 = `TONE` (low-pass IR), 3 = `SIZE` (posun modálních frekvencí = větší/menší tělo). `MIX` je čistě runtime (wet level), zatímco `DECAY`/`TONE`/`SIZE` se aplikují přestavbou IR (`rebuildIr()` off-RT na GUI threadu: SIZE resample → DECAY okno → TONE LP → **energetický ořez ocasu** → `setIR` s 2-slot atomickým swapem). IR výběr přes choice-rozšíření (viz výše). `hasEnable=true`.

**Výkonové optimalizace (sluchově transparentní):** (1) **Head+tail split** hot loopu bez wraparound branche → autovektorizace. (2) `engine/dsp/convolver.cpp` se kompiluje s `-ffast-math`/`/fp:fast` (jen tento TU) → SIMD float-redukce; denormaly kryje globální FTZ/DAZ. (3) **Energetický ořez IR** (`trimmedIrLength`, drop ≤ 1e-6 energie = ~−60 dB): runtime IR (modal/WAV preset) se zkrátí o ~nulový doznívající ocas → méně MAC/vzorek. Baseline: modální base IR má `min(kMaxIr, 0,15·sr)` = 7200 vzorků @48 kHz; default DECAY 0,5 → ~6060 tapů ≈ −16 % (úspora −26 % platila jen proti plnému `kMaxIr` 8192), nízký DECAY ořeže výrazně víc. Přímý `setIR` (raw IR/testy) se neořezává. (4) **Early-out při MIX≈0**: čistý dry, jen O(1) update historie. Garance kvality: ořez i fast-math jsou pod prahem slyšitelnosti (`tests/test_convolver.cpp` ověřuje energetickou záruku ořezu a ekvivalenci s referenční FIR < 1e-3). Reference: RR_9530 (kalibrace `f_ve`), Bank & Chabassier 2019 (soundboard IR je šumovité, krátký doznív → FIR), Teng 2012 (caveat: broadband IR přebíjí vysoké tóny → subtilní MIX). Verifikace: `tests/test_ir_modal.cpp` (decay-vs-f, obálka), `tests/test_convolver.cpp` (impulz→IR, identity→průchod, MIX bypass).
...
**Core — `Engine::processBlock`** (`engine/engine.cpp`): Volá
`dsp_.process(out_l, out_r, n_samples)` jako krok po master gainu a
...
Convolver `mix_`/`decay_`/`tone_`/`size_`, `enabled_`) jsou
`std::atomic<float/bool>` s `memory_order_relaxed` na obou stranách. Přepočet
odvozených koeficientů (biquad, decay, threshold lin) probíhá výhradně na audio
threadu v `process()` — koeficienty tedy nejsou sdíleny, proto pro ně `atomic`
není potřeba. Metrické atomiky (`cur_gain_`, `gr_db_`) jsou psány audio threadem
a čteny GUI threadem, opět s `relaxed` (jde o best-effort zobrazení, ne o
synchronizaci dat).

**Multithreading — Convolver IR swap**: Výjimkou z čistě atomického vzoru je
Convolver: `set(1..3)` (DECAY/TONE/SIZE) a `selectChoice()` spouštějí
`rebuildIr()` přímo na volajícím (GUI) threadu — alokace a výpočty IR (resample,
okno, LP, ořez) běží off-RT, audio thread se jich nikdy neúčastní. Publikace
nového IR do audio threadu probíhá 2-slot swapem s **ack protokolem**: `process()`
hned na začátku zapíše čtený slot do `seen_` (release) a na všech návratových
cestách vrátí −1 („audio nečte"); `setIR` před přepsáním neaktivního slotu
bounded-čeká (yield smyčka, deadline 250 ms kryje zastavené/visící audio), dokud
`seen_ == inactive`, teprve pak move-assignuje nový vector a atomicky publikuje
`active_` (release/acquire pár). Dvě publikace během jednoho in-flight bloku tak
nemohou dealokovat vector pod rukama audio threadu. `prepare()` resetuje `seen_`
na −1 (audio stojí).
...
**6. Convolver: 2-slot IR swap bez potvrzení od audio vlákna — ✅ VYŘEŠENO
(fix/revize-2026-06-10).** Původní swap publikoval `active_` bez ack: dvě rychlé
publikace během jednoho in-flight `process()` mohly move-assignem dealokovat
vector, který audio thread právě četl (latentní use-after-free; viz revize
2026-06-10 §1.7). Opraveno ack protokolem přes atomic `seen_` — `process()`
publikuje čtený slot a na všech návratových cestách vrací −1, `setIR` před
přepsáním neaktivního slotu bounded-čeká (deadline 250 ms), dokud ho audio
thread čte. Viz sekce „Multithreading — Convolver IR swap".
