...
|---|---|---|---|---|---|---|
| `static std::vector<std::string> listPorts()` | off-RT (GUI / CLI) | — → seznam názvů portů | `app/cli/main.cpp`, `app/gui/app_context.cpp`, `app/gui/page_sys.cpp` | `RtMidiIn::getPortName()` — enumerace portů | — | Vytvoří dočasný `RtMidiIn`, projde dostupné porty a vrátí jejich jména. Výjimka z RtMidi je zachycena; v takovém případě se vrátí prázdný nebo částečný seznam. Slouží jen pro zobrazení v UI a CLI (`--midi-list`). |
| `static bool maskAccepts(uint16_t mask, uint8_t status)` | libovolné | `mask`, `status` byte → bool | `callback()` | — | `mask`: bit *i* = kanál *i*+1 propouští; 0xFFFF = OMNI, 0 = nic. `status`: raw MIDI status byte. | Čistá funkce bez vedlejšího efektu: `(mask >> (status & 0x0F)) & 1`. Prázdná maska nepropustí nic — to je legitimní volba uživatele (všechny kanály zhasnuté na SYS), ne chyba. Testovatelná samostatně bez instance. |
| `static bool channelAccepts(int channel, uint8_t status)` | libovolné | `channel` (-1 nebo 0–15), `status` byte → bool | testy | — | `channel`: -1 = OMNI, 0–15 = konkrétní kanál (0-based). | Starší jednokanálová varianta. Filtr v `callback()` už jede přes `maskAccepts`; tahle zůstává jako čitelný ekvivalent pro testy. |
| `void setChannelMask(uint16_t mask)` | off-RT (GUI / CLI) | `mask` → nastaví `mask_` | `app/gui/app_context.cpp` (před open), `app/gui/page_sys.cpp` (za běhu), `app/cli/main.cpp` | `mask_.store(relaxed)` | `mask`: bit *i* = kanál *i*+1. | `mask_` je `std::atomic<uint16_t>` — GUI smí masku měnit i za otevřeného portu, RtMidi callback ji čte jedním atomic loadem per event (plain hodnota by byla formální data race). `app_context.cpp` masku nastavuje **před** `open()`, aby callback hned od otevření filtroval správně. OMNI není zvláštní režim, je to maska se všemi bity — na panelu si ji uživatel naklikne zapnutím všech kanálů, což zároveň dovoluje libovolnou podmnožinu. |
| `void setChannel(int ch)` | off-RT (GUI / CLI) | `ch` → maska | `app/cli/main.cpp` | `setChannelMask()` | `ch`: -1 (OMNI) nebo 0–15; mimo rozsah → OMNI. | Pohodlný obal: -1 → 0xFFFF, *n* → 1<<*n*. |
| `uint16_t channelMask() const` | libovolné | — → aktuální maska | diagnostika | `mask_.load(relaxed)` | — | Čtení masky bez zámku. |
| `bool isOpen() const` | off-RT | — → bool | `app/gui/app_context.cpp` (shutdown), GUI | — | — | Je port otevřený? Používá se při shutdownu, aby se `close()` nevolal na zavřený port. |
| `const std::string& portName() const` | off-RT | — → název | GUI | — | — | Přesný název otevřeného portu (zapisuje se zpět do `state.json`, protože se hledá podle podretezce). |
