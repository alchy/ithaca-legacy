...
| `dampOnPedalUp(const PedalState&, float engine_sr) → int` noexcept | audio | pedál + sr → počet zafadeovaných | `Engine::processBlock` (drain, Sustain DOWN→UP) | `pedal.isUndamped(N)`, `slot->fadeOut` | **Striktní mute při zvednutí pedálu.** Rychlý `fadeOut` (kResonanceFadeOutMs) každého aktivního hlasu na struně, která už není undamped (nedržená klávesa). Držené struny (`damping=1`) znějí dál. Analogie `VoicePool::releasePendingNotes` pro hlavní hlasy — bez něj se rezonance spoléhala jen na per-blok target (pomalejší 30ms ramp až příští blok) a slyšitelně dohrávala. |
