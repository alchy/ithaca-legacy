...
| `bool loadBank(const std::string& dir)` | off-RT | cesta → `true` pokud načteno ≥1 sample | `reloadBank()`, `app/cli/main.cpp` | `ithaca::loadBank()`, `ithaca::buildResonanceCache()`, `resonance_->setCacheReady()` | `dir` — adresář banky | Deleguje na volnou funkci `ithaca::loadBank` s `cfg_.midi_from/to`, `preload_ms`, `resonance_window_ms`. Výsledek zapíše do `bank_`, zacachuje min/max peak RMS (pro GUI slider) a postaví **RAM cache rezonance** (`buildResonanceCache(bank_, cfg_.resonance_layer_db, cfg_.resonance_window_ms)` → 12 s jen cílová vrstva) + nastaví ready flagy. Vrátí false pokud `loaded_samples == 0`. |
