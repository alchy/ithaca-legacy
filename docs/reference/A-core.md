...
| `EngineDiag diag() const noexcept` | GUI | — → POD snapshot | `app/gui/screen.cpp` (jednou za frame → `PanelState::diag`) | tytéž atomic loady jako jednotlivé gettery | — | **Kanonická čtecí cesta GUI.** Hlasy, ringy, peaky, DSP load, pedál, stáří událostí (note-on/off, overload, underruny; `kNever` = nenastala) a fakta o bance (`bank_type`, `loaded_samples`, `recorded_notes`). Nové diagnostické údaje přibývají jen sem. |
...
| `int resonanceVoices() const noexcept` | GUI | — → počet | `app/gui/page_play.cpp` | `resonance_->activeCount()` | — | Triviální diagnostický getter; null-safe. |
| `int numRingsUsed() const noexcept` | GUI | — → součet ringů obou poolů | testy | `stream_main_->numRingsUsed()`, `stream_resonance_->numRingsUsed()` | — | Součet across obou stream poolů; per-pool hodnoty jsou v `EngineDiag`. |
...
| `void setMasterGain(float g)` | GUI | g (lin) | `app/gui/master_page.h` | `master_gain_.store()` | `g` — lineární gain | Atomický store, memory_order_relaxed. |
| `void setReleaseMs(float ms) noexcept` | GUI | ms → (clamped 1..60000) | `app/gui/master_page.h` | `cfg_.release_ms =` | `ms` — release v ms | Zápis do plain float; race s audio vláknem benigní na x86/arm (4-byte aligned write). |
| `void setResonanceGainDb(float db) noexcept` | GUI | dB | `app/gui/resonance_page.h` | `resonance_->setGainDb(db)` | `db` — gain rezonance v dB | Deleguje; ResonanceEngine uloží `10^(db/20)` jako `gain_lin_`. |
...
| `void setResonanceEnabled(bool on) noexcept` | GUI | bool | `app/gui/resonance_page.h`, `app_context.cpp` | `resonance_->setEnabled(on)` | `on` | Deleguje; vypnutí → `onPlayedNoteOn` early-return. |
...
| `float dspLoadPeak() const noexcept` | GUI | — → 0..∞ | GUI (sloupec `DSP` na PLAY) | `dsp_load_peak_.load()` | — | Peak-hold zatížení audio threadu = čas renderu / perioda bloku. 1.0 = hranice deadline. Relaxed. |
| `void scopeSnapshot(float* dst_l, float* dst_r, int n) const noexcept` | GUI | `n` vzorků → zkopíruje posledních `n` (nejstarší první) | GUI (vlna v pozadí, `screen.cpp`) | — | `n` se ořízne na `kScopeSize` (1024, ~21 ms @ 48 kHz) | Kruhový buffer výstupu **za** master gainem i DSP řetězcem — tedy přesně to, co jde do převodníku. Audio vlákno píše (single producer), GUI čte; **záměrně bez zámku** — případné roztržení snímku je na ambientním pozadí neviditelné, kÞžto zámek v audio cestě by byl mnohem horší. |
| `bool overloadRecent(float ms) const noexcept` | GUI | ms → bool | GUI (kontrolka `CLIP` / přebarvení vlny) | `nowMicros()` | `ms` = okno | `true` když overload (load ≥ 1.0) nastal před méně než `ms`. Okno volí konzument; GUI čte stáří z `EngineDiag::overload_age_ms`. |
...
| `void setMinSeverity(Severity s)` | GUI / off-RT | `s` | `app/cli/main.cpp`, `app/gui/app_context.cpp`, `app/gui/page_log.cpp` | `min_severity_.store()` | `s` — minimální severity pro průchod filtrem | Atomický store relaxed; efekt okamžitý (nezamyká). |
