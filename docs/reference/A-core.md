...
| `bool loadBank(const std::string& dir)` | off-RT | cesta → `true` pokud načteno ≥1 sample | `reloadBank()`, `app/cli/main.cpp` | `ithaca::loadBank()`, `ithaca::buildResonanceCache()`, `resonance_->setCacheReady()` | `dir` — adresář banky | Deleguje na volnou funkci `ithaca::loadBank` s `cfg_.midi_from/to`, `preload_ms`, `resonance_window_ms`. Výsledek zapíše do `bank_`, zacachuje min/max peak RMS (pro GUI slider) a postaví **RAM cache rezonance** (`buildResonanceCache(bank_, cfg_.resonance_layer_db, cfg_.resonance_window_ms)` → 12 s jen cílová vrstva) + nastaví ready flagy. Vrátí false pokud `loaded_samples == 0`. **OOM guard:** spočítá efektivní RAM budget (`cfg_.cache_budget_mb`, jinak auto ~60 % fyzické RAM přes `util/sysinfo.h`) a předá ho do `loadBank` (ten nacítání přeruší při překročení). Celé tělo je v `try/catch(std::bad_alloc)` — při nedostatku RAM zaloguje error, zahodí banku (`bank_ = {}`, `clearCacheReady`) a vrátí false **místo pádu**. |
| `bool reloadBank(const std::string& dir)` | off-RT (blokující ~60 ms) | cesta → `bool` | `app/gui/panel_bank.cpp` | `allNotesOff()`, `pool_->reset()`, `resonance_->reset()`, `loadBank()` | `dir` — nový adresář banky | **Graceful reload** v 7 krocích: (1) push AllNotesOff do MIDI fronty; (2) sleep 50 ms (release ramp); (3) `bank_loading_ = true` (audio vlákno vidí a vrací ticho); (4) sleep 10 ms (in-flight processBlock dobíhá); (5) tvrdě zastaví pool_ i resonance_ (`reset()`), aby žádný Voice nedržel ukazatel do staré banky, a **joinne `recache_thread_`** (background rebuild rezonanční cache nesmí běžet při reloadu); (6) `loadBank(dir)` (disk I/O, audio mlčí; znovu postaví rezonanční cache); (7) `bank_loading_ = false`. Volat **pouze** z non-RT vlákna. |
| `void noteOn(int midi, int velocity, int channel = 0)` | GUI / MIDI callback | midi 0–127, vel 1–127, kanál 0–15 | `app/cli/main.cpp`, `MidiInput::callback`, `batch_renderer` | `midi_q_.push()`, `last_note_on_us_.store()` | `midi` — MIDI číslo noty; `velocity` — 1–127; `channel` — MIDI kanál (default 0 pro CLI/batch/testy) | Pokud `velocity ≤ 0`, deleguje na `noteOff(midi, channel)`. Jinak zapíše časové razítko a vloží `NoteOn` event (vč. kanálu) do lock-free MIDI fronty. Thread-safe. |
| `void noteOff(int midi, int channel = 0)` | GUI / MIDI callback | midi 0–127, kanál 0–15 | `app/cli/main.cpp`, `MidiInput::callback`, `batch_renderer` | `midi_q_.push()`, `last_note_off_us_.store()` | `midi` — MIDI číslo noty; `channel` — MIDI kanál | Zapíše časové razítko a vloží `NoteOff` (vč. kanálu) do fronty. Thread-safe. |
...
| `void rebuildResonanceCache(float target_db) noexcept` | GUI (debounced) | nový cíl dB → přestavba cache | `app/gui/main.cpp` (debounce 400 ms) | `resonance_->setLayerTargetDb/clearCacheReady/requestRecacheFade`, `buildResonanceCache` (na `recache_thread_`) | `target_db` — nový layer cíl | **Runtime přestavba RAM cache rezonance.** Aktualizuje `cfg_.resonance_layer_db` + layer target (nové spawny hned berou novou vrstvu), `clearCacheReady` (nové hlasy → stream mód), `requestRecacheFade` (audio vlákno fadene aktivní cache-mód hlasy). Pak na **background vlákně** (`recache_thread_`): ~120 ms čeká na doběh fade, zavolá `buildResonanceCache` a `setCacheReady`. Coalescing přes `recache_running_`/`recache_pending_target_`. Vlákno joinováno v `~Engine` a `reloadBank`. |
...
- `NoteOff` → logguje (vč. `ch`, `last`); `hold_.noteOff(m, ch)`; **jen pokud poslední držitel** → `pedal_.noteOff(m)`, `pool_->noteOffWithPedal(m, pedal_, ...)`. Jinak nota zní dál (jiný kanál ji drží).
