...
|---|---|---|
| `main.cpp` | Vstupní bod: CLI parse, GLFW/ImGui init, DPI scale, render loop, layout shell, modální bank-load overlay, persistence debounce, shutdown sekvence | `GuiState`, `AppContext`, `MasterPage`, `ResonancePage`, `IParamPage*[6]` |
| `app_context.h` / `app_context.cpp` | Vlastník všech těžkých objektů (engine, audio device, MIDI, log buffer); inicializace z persistovaného stavu a shutdown; async bank reload worker (`requestBankReload` / `pollReloadCompletion`) | `AppContext`, `audioCallback()`, `BankLoadProgress` |
...
| `panel_bank.{h,cpp}` | Levý sloupec: výběr banky z adresáře, TYPE badge, statistiky, badge „NEUPLNA (RAM limit)", RELOAD | `renderBankPanel()`, `scanBanks()` |
...
7. **Resonance Layer debounce** — změna `ctx.state.resonance_layer_db` (slider RESONANCE LAYER) zaznamená `layer_dirty_since`; po 400 ms ticha se volá `engine.rebuildResonanceCache(layer_db)` — přestavba RAM cache rezonance na pozadí (background thread enginu, GUI neblokuje). Stejnou cestou projde i layer heuristika „1/3 rozsahu banky" aplikovaná v `pollReloadCompletion` (změní `state` hodnotu → debounce → rebuild).

8. **Async bank reload — completion + modální overlay** — `ctx.pollReloadCompletion()` (převzetí výsledku na GUI vlákně, viz `app_context`). Při `ctx.reloadInProgress()` se kreslí **fullscreen modální overlay**: topmost okno přes celou plochu (`SetNextWindowPos({0,0})` + `SetNextWindowSize({W,H})` + `SetNextWindowFocus()` → pohltí veškerý vstup; poloprůhledné tmavé pozadí). Obsah: název banky (gold), `ImGui::ProgressBar` s frakcí `ithaca::bankLoadFraction(phase, done, total)` (heads = 0–0.6, rezonanční cache = 0.6–1.0), text fáze s čítači („Prohledavam banku..." / „Nacitam samply (done/total)" / „Stavim rezonancni cache (done/total)"), řádek paměti „RAM: X / Y MB (budget)" z `bytes_loaded`/`budget_bytes` (bez budgetu jen „RAM: X MB") a při `truncated` červené varování „Banka prekrocila RAM budget — nactena NEUPLNA (detail v LOG)". Všechny hodnoty se čtou z atomik `ctx.loadProgress()` (relaxed).

9. **OpenGL render** — `glViewport`, `glClear(0.1, 0.1, 0.1)`, `RenderDrawData`, `SwapBuffers`.
...
| `AppContext::shutdown() → void` | GUI | — | `main()` po render loop | `reload_thread_.join()`, `midi.close()`, `audio->stop()`, `Logger::clearSubscribers()` | — | Pořadí shutdown: (0) **join `reload_thread_`** — běžící bank reload nelze přerušit (drží `bank_loading_`), musí doběhnout PŘED zavřením MIDI/audia (quiesce handshake v `reloadBank` využívá běžící audio); (1) MIDI close — zastaví MIDI callback thread, žádné nové noty do fronty; (2) audio stop — zastaví miniaudio callback, engine už nikdo nevolá; (3) clear subscribers — žádné log události se nepokusí sahnout na `log_buf` po jeho potenciální destrukci. |
| `AppContext::requestBankReload(const std::string& dir) → void` | GUI | cesta k bance → spawn worker threadu | `initFromState()` (startovní async load), `renderBankPanel()` (Selectable + RELOAD) | `engine.reloadBank(dir, &load_progress_)` na `reload_thread_` | dir: adresář banky | **Async reload banky.** CAS guard `reload_in_progress_` — druhé volání během běhu je no-op (modální overlay stejně blokuje UI). Joinne případný doběhlý předchozí thread, vyresetuje `load_progress_` (phase/done/total/bytes_loaded/truncated), zapíše `reload_dir_` PŘED spawnem (worker ho čte bez zámku) a spustí `std::thread`, který volá `engine.reloadBank(reload_dir_, &load_progress_)`; po doběhu worker nastaví `reload_ok_` + `reload_done_pending_` a shodí `reload_in_progress_`. Engine ochrany (`bank_loading_`, epoch handshake) zůstávají beze změny. |
| `AppContext::pollReloadCompletion() → void` | GUI (per frame) | — | `main()` render loop (krok 8) | `engine.bankPeakRmsMinDb/MaxDb()`, `Logger::log` | — | **Completion handler na GUI vlákně.** `reload_done_pending_.exchange(false)` — zpracuje se právě jednou. Převezme `load_progress_.truncated` do `bank_truncated_` (badge v BANK panelu). Neúspěch → warning do logu. Úspěch → aplikuje **layer heuristiku „1/3 rozsahu banky"**: jen když `state.resonance_layer_db == -30` (persistovaný default = „uživatel ještě nenastavil"), nastaví `lo + (hi − lo)/3` z rozsahu banky; změna `state` hodnoty spustí existující 400ms layer debounce v `main.cpp` → `engine.rebuildResonanceCache` — heuristika tak nově projde i rebuilem cache (dřív v `initFromState` běžela bez něj, nález H-střední z revize). Nakonec zaloguje výsledek (vč. „NEUPLNA (RAM budget)" při truncated). |
| `AppContext::reloadInProgress() → bool` | GUI | — → bool | `main()` (overlay) | `reload_in_progress_.load(acquire)` | — | True dokud běží reload worker; řídí kreslení modálního overlay. |
| `AppContext::loadProgress() → const BankLoadProgress&` | GUI | — → ref na atomiky | `main()` (overlay) | — | — | Přístup k progress atomikům (`phase`/`done`/`total`/`bytes_loaded`/`budget_bytes`/`truncated`) pro overlay; definice struktury v `engine/sample/sample_store.h`, viz [F-loader](F-loader.md). |
...
5. **Explicitní engine settery** — `engine.setMaxResonanceVoices` (přestože hodnota byla předána přes `cfg`; exercizuje setter cestu), `engine.setResonanceEnabled`, `engine.setResonanceGainDb`, `engine.setResonanceLayerDb(state.resonance_layer_db)` — persistovaná hodnota; heuristika „1/3 rozsahu banky" se zde už NEaplikuje (banka v tu chvíli není načtená) — přesunula se do `pollReloadCompletion()`.
6. **Audio device start** — `audio->start(&audioCallback, &engine, cfg.sample_rate, cfg.block_size)` (validované hodnoty z kroku 3, ne hardcoded). Musí být *po* `engine.init()` (voice pool / stream / ringy jsou připravené). Selhání = warning (GUI stále funguje bez zvuku).
7. **MIDI otevření** — substring match `state.midi_port_name` v `MidiInput::listPorts()`. Při shodě: `midi.setChannel(state.midi_channel)` **před** `midi.open(engine, i)` (callback může běžet hned po otevření portu a filtroval by podle starého kanálu), pak uloží přesný název portu do `state.midi_port_name`. Selhání = warning.
8. **Startovní async bank load** — na konci initu `requestBankReload(state.bank_path)`, jen při neprázdném `bank_path`. Banka se **nenačítá synchronně**: jede stejnou worker cestou jako runtime reload — okno se ukáže hned, průběh kryje modální overlay v render smyčce. Selhání = warning v completion handleru, engine běží prázdný (uživatel vybere banku v UI). Nevrací `false`.
...
| `renderBankPanel(AppContext& ctx) → void` | GUI | — | `main()` render loop | `scanBanks()`, `ctx.requestBankReload()`, `engine.bankType()`, `engine.recordedNotes()`, `engine.loadedSamples()` | — | Vykreslí: eyebrow BANK, dropdown se seznamem kandidátů (filename jako label), TYPE badge (`FIXED`/`DYNAMIC`/`EXTENDED`/`—` ze `engine.bankType()`), statistiky (`X not · Y samplu`), volitelný badge NEÚPLNÁ, RELOAD tlačítko. Výběr jiné banky (Selectable): `state.bank_path = b`, `ctx.requestBankReload(b)` — **async** na worker threadu, GUI neblokuje; průběh kryje modální overlay v `main.cpp`. RELOAD tlačítko volá `ctx.requestBankReload(state.bank_path)`. Pod fakty o bance se při `ctx.bank_truncated_` kreslí zlatý badge **„NEUPLNA (RAM limit)"** — poslední load překročil RAM budget (detail je v LOG stripu z loggeru). |
...
| **Loader / Bank reload** (`engine/engine.h::reloadBank`) | `renderBankPanel` (Selectable + RELOAD) i `initFromState` (startovní load) volají `ctx.requestBankReload(path)` — **async**: worker thread `AppContext` volá `engine.reloadBank(path, &load_progress_)` (drain → silence → load → resume), GUI vlákno neblokuje. Průběh hlásí atomiky `BankLoadProgress` (modální overlay, `bankLoadFraction`), completion zpracuje `pollReloadCompletion()` na GUI vlákně. Viz [F-loader](F-loader.md). |
...
### 3. `reloadBank` blokuje GUI vlákno po dobu disk loadu — ✅ VYŘEŠENO (async loader, 2026-06)

Původní stav: `engine.reloadBank()` (z `renderBankPanel`) byl synchronní — `loadBank` dělal disk I/O **na GUI vlákně**, u větších bank sekundy bez framů.

**VYŘEŠENO (branch `feat/async-loader-a-stream-refactor`):** `requestBankReload` spouští `engine.reloadBank(dir, &load_progress_)` na worker threadu `AppContext::reload_thread_`; GUI vlákno kreslí dál (modální overlay s progress barem, RAM infem a truncated varováním — render loop krok 8), completion zpracuje `pollReloadCompletion()` na GUI vlákně. Startovní load v `initFromState` jede stejnou async cestou (okno se ukáže hned). Loader navíc čte disk paralelně (worker pool, viz [F-loader](F-loader.md)) — samotný load je několikanásobně rychlejší. `shutdown()` reload thread joinuje před zavřením MIDI/audia.
